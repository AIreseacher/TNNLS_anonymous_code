#!/usr/bin/env bash
# ===============================================================
# Ablation: SRA "Fixed-Alpha (0.50)" across datasets
...
LOG_MODE="w"                       # Log both train/val/test
# DATASETS=("Chaoyang" "Kidney" "Breast")
DATASETS=("Brain")
...
for DS in "${DATASETS[@]}"; do
  for PAIR in "${TEACHER_STUDENT_PAIRS[@]}"; do
    ...
    for SEED in "${SEED_ARR[@]}"; do
      SAVE_DIR="checkpoints/${DS}/${TEACHER_STUDENT}/fixed_alpha_050/seed_${SEED}"
      mkdir -p "${SAVE_DIR}"
...
PY
        ...
        if [[ "${LAST_EPOCH}" -ge "${TARGET_EPOCHS}" ]]; then
          echo "[SKIP] ${DS} ${TEACHER_STUDENT} fixed_alpha_050 seed=${SEED}: finished (${LAST_EPOCH}/${TARGET_EPOCHS})"
          continue
        else
          echo "[RESUME] ${DS} ${TEACHER_STUDENT} fixed_alpha_050 seed=${SEED}: resume at ${LAST_EPOCH}/${TARGET_EPOCHS}"
        fi
      fi
      ...
      CMD=(python main_ACC.py
        ...
        # SRA: enable relations, use fixed-alpha variant (alpha=0.50)
        --override "scope.with_relations=True"
        --override "scope.enable_sra=True"
        --override "scope.sra.variant=fixed_alpha"
        --override "scope.sra.alpha=0.5"
      ...
      echo "======================================================"
      echo "   SRA Fixed-Alpha (0.50) Ablation (auto-resume)"
      echo "   Dataset : ${DS}"
