#!/usr/bin/env bash
# ===============================================================
# Ablation: GCR threshold (tau) via gcr_threshold.py (monkey-patch)
# - No changes to main_ACC.py / gcr.py
# - Auto resume / auto skip using checkpoints/last.pth
# - Save under .../<Dataset>/<Pair>/gcr_tau{X}/seed_{seed}
# ===============================================================

set -euo pipefail

CONFIG="./configs/Train.yaml"
JSON_DIR="./json"
GPUS="0"
EPOCHS=""          # Optional override: train.epochs
BATCH_SIZE=""      # Optional override
NUM_WORKERS=""     # Optional override
LOG_MODE="w"

# -------- Edit as needed --------
# DATASETS=("Chaoyang" "Kidney" "Breast")
DATASETS=("Breast" ""Brain"")
TEACHER_STUDENT_PAIRS=(
  "ResNet101 ResNet18"
  "ViT-B ResNet18"
  "WideResNet101 ResNet18"
  "ResNet101 ShuffleNetV2"
)
TAUS=(-0.20 -0.10 0.10 0.20)   # cos(g_kd, g_ce) < tau triggers projection
# --------------------------------

# ---------- Extract seed list from YAML (compatible with int/list) ----------
SEEDS_CSV="$(
python3 - "${CONFIG}" <<'PY'
import sys, yaml
cfg = yaml.safe_load(open(sys.argv[1], 'r')) or {}
train = cfg.get('train', {}) or {}
seeds = train.get('seeds') or [train.get('seed', 42)]
if isinstance(seeds, int): seeds = [seeds]
print(",".join(map(str, seeds)))
PY
)"
IFS=',' read -r -a SEED_ARR <<< "${SEEDS_CSV}"
echo "[INFO] Seeds: ${SEEDS_CSV}"

export CUDA_VISIBLE_DEVICES="${GPUS}"

# ---------- Helper: read target epochs from YAML unless overridden ----------
read_target_epochs() {
  if [[ -n "${EPOCHS}" ]]; then
    echo "${EPOCHS}"
  else
    python3 - "${CONFIG}" <<'PY'
import sys, yaml
cfg = yaml.safe_load(open(sys.argv[1], 'r')) or {}
print((cfg.get('train', {}) or {}).get('epochs', 0))
PY
  fi
}
TARGET_EPOCHS="$(read_target_epochs)"
echo "[INFO] Target epochs: ${TARGET_EPOCHS}"

for DS in "${DATASETS[@]}"; do
  for PAIR in "${TEACHER_STUDENT_PAIRS[@]}"; do
    set -- $PAIR; TEACHER="$1"; STUDENT="$2"
    TEACHER_STUDENT="${TEACHER}_${STUDENT}"
    for TAU in "${TAUS[@]}"; do
      # folder-friendly name: 0.10 -> 0p10
      TAU_TAG="gcr_tau$(echo "${TAU}" | sed 's/\./p/g')"
      for SEED in "${SEED_ARR[@]}"; do
        SAVE_DIR="checkpoints/${DS}/${TEACHER_STUDENT}/${TAU_TAG}/seed_${SEED}"
        mkdir -p "${SAVE_DIR}"
        LAST_PTH="${SAVE_DIR}/last.pth"

        # ---------- Auto resume / auto skip ----------
        if [[ -f "${LAST_PTH}" ]]; then
          LAST_EPOCH="$(python3 - <<PY
import torch
ckpt = torch.load("${LAST_PTH}", map_location="cpu")
print(ckpt.get("epoch", -1))
PY
)"
          if [[ "${LAST_EPOCH}" -ge "${TARGET_EPOCHS}" ]]; then
            echo "[SKIP] ${DS} ${TEACHER_STUDENT} ${TAU_TAG} seed=${SEED}: finished (${LAST_EPOCH}/${TARGET_EPOCHS})"
            continue
          else
            echo "[RESUME] ${DS} ${TEACHER_STUDENT} ${TAU_TAG} seed=${SEED}: resume at ${LAST_EPOCH}/${TARGET_EPOCHS}"
          fi
        fi

        CMD=(python gcr_threshold.py
          --config "${CONFIG}"
          --json_dir "${JSON_DIR}"
          --seed "${SEED}"
          --log_mode "${LOG_MODE}"
          --override "data.dataset=${DS}"
          --override "output.save_dir=${SAVE_DIR}"

          # Keep SRA full (consistent with your fixed setup)
          --override "scope.with_relations=True"
          --override "scope.enable_sra=True"
          --override "scope.sra.variant=full"
          --override "scope.sra.alpha_mode=entropy"

          # Ensure GCR path is used & log cos similarity
          --override "gcr.variant=gcr"
          --override "gcr.log_cos=True"
          --override "gcr.sample_epochs=0.1,0.5,0.9"
          --override "gcr.sample_batches=16"

          # Threshold control (monkey-patched in gcr_threshold.py)
          --gcr_tau "${TAU}"         # cos-mode: project if cos < tau
          # --gcr_mode dot           # (optional) switch to dot-product mode
        )

        [[ -n "${EPOCHS}" ]]      && CMD+=(--override "train.epochs=${EPOCHS}")
        [[ -n "${BATCH_SIZE}" ]]  && CMD+=(--override "data.batch_size=${BATCH_SIZE}")
        [[ -n "${NUM_WORKERS}" ]] && CMD+=(--override "data.num_workers=${NUM_WORKERS}")

        echo "==== ${TAU_TAG} | ${DS} | ${TEACHER_STUDENT} | seed ${SEED} ===="
        "${CMD[@]}"
      done
    done
  done
done
