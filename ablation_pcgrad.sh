#!/usr/bin/env bash
# ===============================================================
# Ablation: PCGrad as the gradient resolution strategy
...
for DS in "${DATASETS[@]}"; do
  for PAIR in "${TEACHER_STUDENT_PAIRS[@]}"; do
    ...
    for SEED in "${SEED_ARR[@]}"; do
      SAVE_DIR="checkpoints/${DS}/${TEACHER_STUDENT}/pcgrad/seed_${SEED}"
      mkdir -p "${SAVE_DIR}"
...
PY
        ...
        if [[ "${LAST_EPOCH}" -ge "${TARGET_EPOCHS}" ]]; then
          echo "[SKIP] ${DS} ${TEACHER_STUDENT} pcgrad seed=${SEED}: finished (${LAST_EPOCH}/${TARGET_EPOCHS})"
          continue
        else
          echo "[RESUME] ${DS} ${TEACHER_STUDENT} pcgrad seed=${SEED}: resume at ${LAST_EPOCH}/${TARGET_EPOCHS}"
        fi
      fi
      ...
      CMD=(python main_ACC.py
        ...
        # Use PCGrad as gradient conflict handler
        --override "gcr.variant=pcgrad"
      ...
      echo "==== PCGrad | ${DS} | ${TEACHER_STUDENT} | seed ${SEED} ===="
      "${CMD[@]}"
    done
  done
done
